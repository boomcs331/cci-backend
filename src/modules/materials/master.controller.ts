import { Controller, Get, Post, Put, Delete, Body, Param, ParseIntPipe, Query, BadRequestException, Patch } from '@nestjs/common';
import { MasterService } from './master.service';
import { MaterialsService } from './materials.service';
import { CreateMasterDto, UpdateMasterDto, PaginationDto } from './dto/master.dto';
import { CreateSupplierDto, UpdateSupplierDto, CreateMaterialsTypeDto, CreateMaterialsLocationDto } from './dto/materials.dto';
...
export class MasterController {
  ...
  @Get('models/:id')
  async getModelById(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
    const model = await this.masterService.findModelById(id);
    return ResponseHelper.success(model, 'Model retrieved successfully');
  }

  @Patch('models/:id')
  async updateModel(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number, @Body() dto: UpdateMasterDto) {
    const model = await this.masterService.updateModel(id, dto);
    return ResponseHelper.success(model, 'Model updated successfully');
  }

  @Delete('models/:id')
  async deleteModel(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
  ...
  async createDeliveryType(@Body() dto: CreateMasterDto) {
    ...
    return ResponseHelper.success(deliveryType, 'Delivery type created successfully');
  }
  ...
  async getDeliveryTypeById(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
    const deliveryType = await this.masterService.findDeliveryTypeById(id);
    return ResponseHelper.success(deliveryType, 'Delivery type retrieved successfully');
  }

  @Put('delivery-types/:id')
  async updateDeliveryType(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number, @Body() dto: UpdateMasterDto) {
    const deliveryType = await this.masterService.updateDeliveryType(id, dto);
    return ResponseHelper.success(deliveryType, 'Delivery type updated successfully');
  }

  @Delete('delivery-types/:id')
  async deleteDeliveryType(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
  ...
  @Get('units/:id')
  async getUnitById(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
    const unit = await this.masterService.findUnitById(id);
    return ResponseHelper.success(unit, 'Unit retrieved successfully');
  }

  @Put('units/:id')
  async updateUnit(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number, @Body() dto: UpdateMasterDto) {
    const unit = await this.masterService.updateUnit(id, dto);
    return ResponseHelper.success(unit, 'Unit updated successfully');
  }

  @Delete('units/:id')
  async deleteUnit(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
  ...
  async createLoadingPoint(@Body() dto: CreateMasterDto) {
    ...
    return ResponseHelper.success(loadingPoint, 'Loading point created successfully');
  }
  ...
  async getLoadingPointById(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
    const loadingPoint = await this.masterService.findLoadingPointById(id);
    return ResponseHelper.success(loadingPoint, 'Loading point retrieved successfully');
  }

  @Put('loading-points/:id')
  async updateLoadingPoint(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number, @Body() dto: UpdateMasterDto) {
    const loadingPoint = await this.masterService.updateLoadingPoint(id, dto);
    return ResponseHelper.success(loadingPoint, 'Loading point updated successfully');
  }

  @Delete('loading-points/:id')
  async deleteLoadingPoint(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
  ...
  async createProcessLine(@Body() dto: CreateMasterDto) {
    ...
    return ResponseHelper.success(processLine, 'Process line created successfully');
  }
  ...
  async getProcessLineById(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
    const processLine = await this.masterService.findProcessLineById(id);
    return ResponseHelper.success(processLine, 'Process line retrieved successfully');
  }

  @Put('process-lines/:id')
  async updateProcessLine(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number, @Body() dto: UpdateMasterDto) {
    const processLine = await this.masterService.updateProcessLine(id, dto);
    return ResponseHelper.success(processLine, 'Process line updated successfully');
  }

  @Delete('process-lines/:id')
  async deleteProcessLine(@Param('id', new ParseIntPipe({ errorHttpStatusCode: 400 })) id: number) {
  ...
  async getAllMaterialsTypesForDropdown() {
    const types = await this.materialsService.findAllMaterialsTypesForDropdown();
  ...
  async getAllMaterialsTypes(@Query() pagination: PaginationDto) {
    const result = await this.materialsService.findAllMaterialsTypes(pagination);
    return ResponseHelper.success(result, 'Materials types retrieved successfully');
  }
  ...
  async getMaterialsTypeById(@Param('id', ParseIntPipe) id: number) {
    ...
    return ResponseHelper.success(type, 'Materials type retrieved successfully');
  }

  @Patch('materials-types/:id')
  async updateMaterialsType(@Param('id', ParseIntPipe) id: number, @Body() dto: CreateMaterialsTypeDto) {
  ...
  async getAllMaterialsLocationsForDropdown() {
    const locations = await this.materialsService.findAllMaterialsLocationsForDropdown();
  ...
  async getAllMaterialsLocations(@Query() pagination: PaginationDto) {
    const result = await this.materialsService.findAllMaterialsLocations(pagination);
    return ResponseHelper.success(result, 'Materials locations retrieved successfully');
  }
  ...
  async getMaterialsLocationById(@Param('id', ParseIntPipe) id: number) {
    ...
    return ResponseHelper.success(location, 'Materials location retrieved successfully');
  }

  @Patch('materials-locations/:id')
  async updateMaterialsLocation(@Param('id', ParseIntPipe) id: number, @Body() dto: CreateMaterialsLocationDto) {
    const location = await this.materialsService.updateMaterialsLocation(id, dto);
    return ResponseHelper.success(location, 'Materials location updated successfully');
  }
  ...
  async deleteMaterialsLocation(@Param('id', ParseIntPipe) id: number) {
    ...
    return ResponseHelper.success(null, 'Materials location deleted successfully');
  }
}
